...
public class CreateAdapter {
	...
	@Test
	public void test()  {
		String callCommand = "ember generate adapter %s";
		Root root = new Root("create_adapter", "创建适配器(adapter)","adapter", callCommand, "IProject");
		
		PageControl pageControl1 = new PageControl("adapter_title_label","Label","适配器名称(adapter):","SWT.NONE");
		PageControl pageControl1_1 = new PageControl("0","Text","请输入适配器路径或名称(路径用'/')","SWT.BORDER");
...
//			FileWriter fileWriter = new FileWriter(new File("C:\\Users\\ThinkPad\\Desktop\\json.txt"));
			Writer fileWriter = new BufferedWriter(  
			        new OutputStreamWriter(  
			                new FileOutputStream(new File("C:\\Users\\ThinkPad\\Desktop\\开发向导\\配置文件\\创建适配器.config")), "UTF-8")); 
