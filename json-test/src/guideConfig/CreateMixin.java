...
public class CreateMixin {
	...
	@Test
	public void test()  {
		String callCommand = "ember generate mixin %s";
		Root root = new Root("create_mixin", "创建mixin(mixin)","mixin", callCommand, "IProject");
		
		PageControl pageControl1 = new PageControl("mixin_title_label","Label","mixin名称(mixin):","SWT.NONE");
		PageControl pageControl1_1 = new PageControl("0","Text","请输入mixin路径或名称(路径用'/')","SWT.BORDER");
...
//			FileWriter fileWriter = new FileWriter(new File("C:\\Users\\ThinkPad\\Desktop\\json.txt"));
			Writer fileWriter = new BufferedWriter(  
			        new OutputStreamWriter(  
			                new FileOutputStream(new File("C:\\Users\\ThinkPad\\Desktop\\开发向导\\配置文件\\创建mixin.config")), "UTF-8")); 
