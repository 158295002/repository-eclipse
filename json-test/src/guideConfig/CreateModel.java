...
public class CreateModel {
	...
	@Test
	public void test()  {
		String callCommand = "ember generate model %s";
		Root root = new Root("create_model", "创建模型(model)","model", callCommand, "IProject");
		
		PageControl pageControl1 = new PageControl("model_title_label","Label","模型名称(model):","SWT.NONE");
		PageControl pageControl1_1 = new PageControl("0","Text","请输入模型路径或名称(路径用'/')","SWT.BORDER");
...
//			FileWriter fileWriter = new FileWriter(new File("C:\\Users\\ThinkPad\\Desktop\\json.txt"));
			Writer fileWriter = new BufferedWriter(  
			        new OutputStreamWriter(  
			                new FileOutputStream(new File("C:\\Users\\ThinkPad\\Desktop\\开发向导\\配置文件\\创建模型.config")), "UTF-8")); 
